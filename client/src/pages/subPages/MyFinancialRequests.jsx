...
const MyFinancialRequests = () => {
  ...
  const [filteredFinancialRequestList, setFilteredFinancialRequestList] =
    useState([]);
  ...
  const [selectedRepaymentTerms, setSelectedRepaymentTerms] = useState("");
  ...
  const handleFilterChange = () => {
    const filteredList = financialRequestList.filter((financial) => {
      ...
      const matchesYear = yearFilter
        ? requestYear === parseInt(yearFilter)
        : true;
      const matchesMonth = monthFilter
        ? requestMonth === parseInt(monthFilter)
        : true;
  ...
  const handleRepaymentTermsClick = (repaymentTerms) => {
    setSelectedRepaymentTerms(repaymentTerms);
    setIsModalOpen(true);
  };
  ...
  const closeModal = () => {
    ...
    setSelectedRepaymentTerms("");
  };

  const formatDate = (dateString) => {
    const date = new Date(dateString);
    return date.toLocaleDateString("en-CA"); // Format as YYYY-MM-DD or adjust it as needed
  };
  ...
  return (
    <div className="p-6 px-20 bg-[#eaeaea] rounded-lg shadow-md">
            ...
            {[
              ...new Set(
                financialRequestList.map((financial) =>
                  new Date(financial.date_of_request).getFullYear()
                )
              ),
            ]
              .sort((a, b) => b - a)
              ...
              filteredFinancialRequestList.map((financial, index) => (
                <tr key={index}>
                  ...
                  <td
                    ...
                    onClick={() =>
                      handleRepaymentTermsClick(financial.repayment_terms)
                    }
                  >
      ...
      <Modal
        isOpen={isModalOpen}
        onClose={closeModal}
        repaymentTerms={selectedRepaymentTerms}
      />
    </div>
  );
};
