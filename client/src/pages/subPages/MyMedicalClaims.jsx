...
const MyMedicalClaims = () => {
    ...
    const fetchClaims = async () => {
        try {
            const res = await axios.get(`https://global-hrm-mobile-server.vercel.app/medical/getMedicalClaim/${empId}`);
            setClaims(res.data);
                            ...
                            filteredClaims.map((claim, index) => (
                                <tr key={index} className="hover:bg-gray-50">
                                    ...
                                    <td
                                        ...
                                        ${claim.claimstatus === 'Pending' ? 'text-orange-500' : ''}
                                        ${claim.claimstatus === 'Accepted' ? 'text-green-500' : ''}
                                        ${claim.claimstatus === 'Rejected' ? 'text-red-500' : ''}`
                                        }
                                    >
                                        {claim.claimstatus}
                                    </td>
